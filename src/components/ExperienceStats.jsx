...
export default function ExperienceStats() {
  ...
  const animateCounters = () => {
    ...
    const interval = setInterval(() => {
      ...
      setCounts({
        ...
        countries: Math.floor(2 * progress),
  ...
  return (
            <div className="py-8 sm:py-12 px-4 bg-white/60 dark:bg-gray-900/60 backdrop-blur-sm">
        ...
        <div className="mt-12">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {experience.experiences.slice(0, 4).map((exp, index) => (
              <div key={index} className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-lg p-6 shadow-lg border border-gray-200/50 dark:border-gray-700/50 hover:shadow-xl transition-all duration-300">
                <div className="text-center">
                  ...
                  <h4 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-2">{exp.title}</h4>
                  <p className="text-sm font-medium text-blue-600 dark:text-blue-400 mb-1">{exp.company}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">{exp.location}</p>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mb-3">{exp.period}</p>
                  <div className="bg-blue-50/50 dark:bg-blue-900/50 rounded px-3 py-2">
                    <p className="text-xs text-blue-700 dark:text-blue-300 font-medium">{exp.highlights[0]}</p>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="mt-12 text-center">
            ...
            <div className="flex flex-wrap justify-center gap-6">
              <div className="flex items-center space-x-2 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-lg px-4 py-2 shadow-md">
